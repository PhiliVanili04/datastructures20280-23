    ...
    public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
        ...
        public static void main(String[] args) {
            ...
            for (int i = 0; i < n_trials; ++i) {
                if (rnd.nextFloat() > 0.5) { // 50% chance for either operation
                    if (bst.size() < n_max) { // Ensure the tree does not exceed n_max elements
                        int x = rnd.nextInt(n_max);
                        bst.put(x, x); // Put random value
                    }
                } else {
                    if (!bst.isEmpty()) {
                        ArrayList<Integer> keys = new ArrayList<>((Collection) bst.keySet());
                        int x = keys.get(rnd.nextInt(keys.size()));
                        bst.remove(x); // Remove existing element
                    }
                }
            }
            System.out.println(bst.tree.inorder());
